...
describe('auth integration', () => {
  describe('POST /api/auth/register', () => {
    ...
    it('lets a self-registered student submit an institution token later', async () => {
      const schoolEmail = `late-token-${randomUUID()}@school.test`;
      ...
      const submitResponse = await request(app)
        ...
        .set('Authorization', `Bearer ${registerResponse.body.data.accessToken as string}`)
        .send({ institutionToken: token });

      expect(submitResponse.status).toBe(200);
      expect(submitResponse.body.data.message).toBe(
        'Token submitted successfully. Your institution can now review your account.',
      );
      ...
      const updated = await User.findOne({ email: registerResponse.body.data.user.email }).lean();
  ...
  describe('terms acceptance', () => {
    it('includes the current terms state in login payloads and persists acceptance', async () => {
      const { email } = await createApprovedUser({
        role: UserRole.INVESTOR,
        email: `investor-${randomUUID()}@example.com`,
        displayName: 'Terms Investor',
        domain: 'FinTech',
      });
      ...
      expect(loginResponse.body.data.user.hasAcceptedCurrentTerms).toBe(false);
      ...
      const acceptResponse = await request(app)
        ...
        .send({ version: loginResponse.body.data.user.termsCurrentVersion });

      expect(acceptResponse.status).toBe(200);
      expect(acceptResponse.body.data.hasAcceptedCurrentTerms).toBe(true);
      expect(acceptResponse.body.data.termsAcceptance).toEqual(
        expect.objectContaining({
          version: '2026-04-06',
          sector: UserRole.INVESTOR,
        }),
      );

      const storedUser = await User.findOne({ email }).lean();
      expect(storedUser?.termsAcceptance).toEqual(
        expect.objectContaining({
          version: '2026-04-06',
          sector: UserRole.INVESTOR,
        }),
