...
router.post('/:id/upload-pitch', authorize(UserRole.STUDENT), withRateLimit(uploadLimiter), enforceStorageQuota(), upload.single('file'), asyncHandler(uploadPitchController));
router.post('/:id/documents', authorize(UserRole.STUDENT), withRateLimit(uploadLimiter), enforceStorageQuota(), documentUpload.single('file'), asyncHandler(uploadStartupDocumentController));
