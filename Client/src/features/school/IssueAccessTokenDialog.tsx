  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        ...
        <div className="grid gap-4 rounded-3xl border border-slate-800/70 bg-slate-900/50 p-5 md:grid-cols-[minmax(0,1fr),auto]">
          <div>
          ...
          {tokensQuery.isLoading ? (
            ...
            <div className="space-y-3">
              {latestTokens.map((token) => (
                <div
                  key={token._id}
                  className="flex flex-col gap-3 rounded-2xl border border-slate-800 bg-slate-950/80 px-4 py-4 md:flex-row md:items-center md:justify-between"
                >
                  <div>
