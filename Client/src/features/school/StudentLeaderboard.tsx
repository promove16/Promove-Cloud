  ...
  const createTemporaryCredentialMutation = useMutation({
    ...
    onSuccess: (credential) => {
      ...
      void Promise.all([
        ...
        queryClient.invalidateQueries({ queryKey: ['institution-leaderboard', 'school'] }),
      ]);
    },
    onError: (error) => {
      ...
      headerAction={
        <div className="flex flex-wrap items-center gap-3">
          ...
          <StudentIntakePanel
            ...
            description="Create a school-managed roster and email invites automatically."
          ...
          <InstitutionAccessTokenDialog
            ...
            mode="school"
            title="School Access Tokens"
            description="Issue a token and share it with students who need to register against your school."
            tokenPlaceholder="Grade 12 innovators"
            fallbackLabel="General school onboarding token"
            queryKeyPrefix="school"
            fetchTokens={schoolApi.getStudentAccessTokens}
            createToken={schoolApi.createStudentAccessToken}
          />
        </div>
      }
      sidePanel={
