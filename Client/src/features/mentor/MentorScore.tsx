...
import { MentorTrainingModal } from './MentorTrainingModal';
      ...
      <div className="flex items-center gap-2">
        ...
        {onAction && actionLabel && (
          <button
            type="button"
            onClick={onAction}
            className="rounded-md border border-cyan-500/30 bg-cyan-500/10 px-2 py-0.5 text-[11px] font-medium text-cyan-400 hover:bg-cyan-500/20 transition"
          >
            {actionLabel}
          </button>
        )}
...
export default function MentorScore() {
  ...
  return (
    <div className="space-y-5">
            ...
            <div key={item.id} className="flex items-center gap-3 px-5 py-3">
              ...
              {item.id === 'training' ? (
                <button
                  ...
                  aria-label="Open Mentor Training Quizzes"
                  className="flex-shrink-0 inline-flex items-center gap-1 rounded-lg border border-cyan-500/30 bg-cyan-500/10 px-2.5 py-1 text-xs font-medium text-cyan-400 transition hover:bg-cyan-500/20"
                >
                  Take Quizzes
          ...
          {score ? (
            <div className="divide-y divide-slate-800/60">
              ...
              <PhaseBreakdownRow
                ...
                value={score.phase1Breakdown.training}
                cap={60}
                onAction={() => setIsTrainingModalOpen(true)}
      ...
      {/* Training & Certification Quizzes Modal */}
      <MentorTrainingModal
        isOpen={isTrainingModalOpen}
        onClose={() => setIsTrainingModalOpen(false)}
        trainingPoints={score?.phase1Breakdown.training ?? 0}
