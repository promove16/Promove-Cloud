...
export default function ComplianceReport() {
  ...
  const incidentsQuery = useQuery({
    queryKey: ['college-compliance-incidents'],
    queryFn: () => collegeApi.getComplianceIncidents({ limit: 50 }),
  });

  const alertsQuery = useQuery({
    queryKey: ['college-compliance-alerts'],
    queryFn: () => collegeApi.getComplianceAlerts({ limit: 50 }),
  });

  const actionsQuery = useQuery({
    queryKey: ['college-compliance-actions'],
    queryFn: collegeApi.getComplianceActions,
  });
  ...
  const refreshCompliance = async () => {
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: ['college-compliance-overview'] }),
      queryClient.invalidateQueries({ queryKey: ['college-compliance-incidents'] }),
      queryClient.invalidateQueries({ queryKey: ['college-compliance-alerts'] }),
      queryClient.invalidateQueries({ queryKey: ['college-compliance-actions'] }),
    ]);
  };

  const createIncidentMutation = useMutation({
    mutationFn: collegeApi.createComplianceIncident,
    onSuccess: async () => {
      setIncidentTitle('');
  ...
  const resolveIncidentMutation = useMutation({
    mutationFn: ({ incidentId, status }: { incidentId: string; status: ComplianceIncidentRecord['status'] }) =>
      collegeApi.updateComplianceIncident(incidentId, { status }),
    onSuccess: refreshCompliance,
  });

  const createAlertMutation = useMutation({
    mutationFn: collegeApi.createComplianceAlert,
    onSuccess: async () => {
      setAlertTitle('');
      setAlertMessage('');
      await refreshCompliance();
  ...
  const markAlertReadMutation = useMutation({
    mutationFn: collegeApi.markComplianceAlertRead,
    onSuccess: refreshCompliance,
  });

  const createActionMutation = useMutation({
    mutationFn: collegeApi.createComplianceAction,
    onSuccess: async () => {
      setActionTitle('');
      setActionDetails('');
      await refreshCompliance();
  ...
  const completeActionMutation = useMutation({
    mutationFn: ({ actionId, status }: { actionId: string; status: ComplianceActionRecord['status'] }) =>
      collegeApi.updateComplianceAction(actionId, { status }),
    onSuccess: refreshCompliance,
  });
  ...
  const generateMutation = useMutation({
    ...
    onSuccess: (payload) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['college-latest-report'] });
    },
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white">College Compliance Panel</h1>
          <p className="mt-2 text-slate-400">
            Track accreditation readiness, handle incidents, manage alerts, and close compliance actions.
          </p>
        </div>
