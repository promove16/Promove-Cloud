...
export function DashboardSection({
  ...
  return (
    <section className={`border-t border-slate-800/80 pt-6 ${className}`.trim()}>
...
export function DashboardMetricRail({
  ...
  return (
    <div className={`grid gap-4 border-y border-slate-800/80 py-5 ${columnsClassName}`}>
      {items.map((item, index) => (
        <div
          key={item.label}
          className={`space-y-3 ${index > 0 ? 'md:border-l md:border-slate-800/80 md:pl-5' : ''}`}
        >
...
export function DashboardRow({
  ...
  return (
    <div className={`border-b border-slate-800/70 py-4 last:border-b-0 ${className}`.trim()}>
      {children}
    </div>
  );
}
