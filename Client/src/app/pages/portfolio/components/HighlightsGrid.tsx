...
export function HighlightsGrid({ cards }: HighlightsGridProps) {
  ...
  return (
    <section className="rounded-2xl border border-cyan-500/15 bg-slate-900/70 p-6 shadow-lg shadow-cyan-500/5">
      <h3 className="text-xl font-semibold text-white">Page Highlights</h3>
      <div className="mt-4 grid gap-4 md:grid-cols-3">
        {source.map((card) => (
          <article
            key={card.title}
            className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4 transition duration-200 hover:-translate-y-1 hover:border-cyan-400/40 hover:shadow-lg hover:shadow-cyan-500/10"
          >
